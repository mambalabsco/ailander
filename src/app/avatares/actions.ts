 ...
 *
 * Con el generador barato —el nano banana normal, no el Pro—: son treinta
 * imágenes y la diferencia de calidad no compensa treinta veces el precio en
 * fotos que imitan a propósito una foto de móvil.
 */
export async function generateShotsAction(input: unknown): Promise<LaunchResult> {
  ...
  return runInBackground({
    ...
    revalidate: "/avatares",
    resume: { productId, avatarIds, perAvatar, contexts: contextIds, holding },
    work: async (report) => {
